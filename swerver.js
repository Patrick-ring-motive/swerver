  ...
  globalThis.spellFix = async function() {
    ...
    while (n = walk.nextNode()) {
      ...
      nwords = (await Promise.allSettled(nwords.map(async x => {
        ...
        let res = await nextSwerver().fetch('check', {
          method: 'POST',
          body: x
        });
        ...
        let y = await (await nextSwerver().fetch('suggest', {
          method: 'POST',
          body: x
        })).json();
        console.warn(y);
...
async function startSwerver() {
  ...
  const swerver = new Worker(URL.createObjectURL(new Blob(['(' + Swerver + ')();'], {
    type: "text/javascript"
  })));
  swerver.ready = new Promise((resolve) => {
    swerver.readyResolve = resolve
  });
...
async function Swerver() {
  ...
  requestChannel.receive = async function receive(workerId) {
    ...
    const res = await requestChannel.match(key) ?? new Response(null, {
      status: 404
    });
  ...
  function onRequest(req) {
    return matchRoute(req.url)?.(req) ?? new Response('Not Found', {
      status: 404
    });
  }
  ...
  self.onmessage = async function(event) {
    try {
      ...
      try {
        //console.log(e);
        await responseChannel.put(new Request(location.origin + workerId), new Response(e.message, {
          status: 500
        }));
